...
8. **Delivery.** Trunk is cloned to `projects/<name>/build/`, the run command is
   derived , `spec_baseline_sha` is recorded, and the
   PM tells the client where it is and how to run it.
...
- The client asks in the same chat. The PM edits the affected requirement files **in
  place** — they describe the product as it is now — and records what changed in
...
- `<data root>/forge.db` — the global registry: project names, and the *names* of
  vault secrets (never values).
- `<data root>/projects/<name>/project.db` — everything about one project: queue,
...
- `rejected` and `cancelled` are terminal and kept, never deleted — their ledger rows
  stay attributable.
...
| `QaSuiteAuthor` — project *has* an API contract | Coding | `qa-suite` | write_file, done, escalate — no way to run anything | 500k / 40 |
| `Qa` — project has *no* contract, so QA drives the app itself | Coding | `qa` | write_file, run, serve, stop_server, http, file_bug, how_to_run, done, escalate | 500k / 40 |
...
1. Resolve the model **once per instance** — a conversation must not change model
   underneath itself — and the per-turn output ceiling from the provider.
...
An incomplete design parks the Feature on the client rather than releasing it — design
is not idempotent, so re-running it would duplicate the tasks it already created.
...
The worker is serial and runs one step per tick, in priority order
(`TaskRunner.RunNextByPriorityAsync`): merging → review → Principal-owned → ask the
client → engineer work → close Features → QA. Part-finished work is always cleared
...
| Trigger | Counter | Response |
|---|---|---|
| Instance ends `budget`/`iterations` | `stall_count` +1 | `stalled`; the Principal triages: `redirect`, `break_and_relink`, or `escalate` |
| Instance ends `crash` | crashes since the last completed instance ≤ 2 | Left claimable; auto-resumes |
| 5 engineer attempts that reached the gates | `RevisionCap` | Handed to the Principal at the strike that makes it **implement the task itself** |
| 3 Principal implementation attempts | `PrincipalAttemptCap` | Final triage, with `redirect` withheld |
| `stall_count` > 2 | — | Final triage with the `cut-it-down` play, once; after that the task is **closed short** |
| Nothing landed and no option left | — | `needs_human`: the PM asks the client |

Counted for the life of the task. Nothing an agent does clears either count — otherwise
a reviewer could buy five more attempts by rejecting the work again.

Two rules keep the ladder honest. A spent cap hands the task to a **rung**, never to the
client: why a task cannot pass CI is a technical question the client cannot answer. And
a task closed short still ends `done` — cancelling it would strand every task waiting on
it — with what is missing surfacing where it can be acted on, in QA.

Other recovery moves:

- **`break_and_relink`** replaces an oversized task with the smaller ones just created:
  the harness re-points every dependent at all of them, gives each the old task's
  dependencies, files them under its Feature, and cancels it. Every check runs before the
  first write, so a refusal leaves the graph untouched. Capped at two splits deep.
...
Work parked on `needs_human` is raised by the PM in one unprompted turn, keyed on the
set of waiting task ids so each distinct set is asked about once, in plain language with
no ids or budgets. The client's answer comes back through the PM as `retriage(task,
note)` — which records the guidance, resets the stall count, and sends it to the
Principal — or `cancel_task(task, reason)`. Escalations pending on the PM are injected
into its next turn, so nothing strands behind a message nobody reads.

The loop never blocks on a human: `needs_human` is in no queue.

### Project budget

A spent dollar cap is not a failure. The task is left exactly as it stands — the claim
is rolled back to the status it held, so which role owned it is not erased — no strike is
...
`MaybeRunQaAsync` runs a round when the board is quiescent and `CountDone()` exceeds
`qa_verified_count`. A round that accepts nothing new never moves the watermark, so QA is
not called again and the project is complete. A project that never converges escalates
after 5 rounds.
...
- **With a contract** (`QaSuiteAuthor`): QA writes black-box xUnit tests into
  ...
  model's memory — fixes its name at `AcceptanceTests.csproj`, deletes any stray project
...
- **Without one** — a CLI tool, a library: the exploratory `Qa` recipe drives the app
...
1. Build the suite. A suite that does not compile has tested nothing and must never be
   reported as a suite that failed — a failed run becomes a bug, and rejecting that bug
...
No code names a model. A recipe names a `ModelTier` — `Fast | Coding | Reasoning` — and
...
| Provider | Key | Fast / Coding / Reasoning |
|---|---|---|
| `anthropic` | `ANTHROPIC_API_KEY` | `claude-haiku-4-5` / `claude-sonnet-5` / `claude-opus-4-8` |
| `openai` | `OPENAI_API_KEY` | `gpt-5-nano` / `gpt-5` / `gpt-5.4` |
| `gemini` | `GEMINI_API_KEY` | `gemini-3.1-flash-lite` / `gemini-2.5-flash` / `gemini-3.6-flash` |

Adapters are hand-rolled over `HttpClient`; there is no SDK surface to keep in step.
Adding a provider is one adapter class with a three-entry tier map plus one case in
...
The API is two methods: `Event(EventType, msg)` for typed events — the enum is the only
category argument, so a git merge cannot be mis-tagged as lifecycle — and `Message(msg)`
for the free-form channel. `.For(taskId)` binds the correlation once. Logging is optional
